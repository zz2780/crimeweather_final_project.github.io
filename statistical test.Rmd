...
df_1 <- crime_df %>%
  ...
  group_by(year,mon) %>%
  summarise(total_by_mon = n()) %>%
  pivot_wider(
    names_from = "year",
    values_from = "total_by_mon"
  )%>%
  na.omit()
df_1 %>%
...
```
As the result told us, the p-value > 0.05 , therefore we fail to reject the null 
hypothesis and conclude that the distribution of number of crimes cases
of each month is the samein 2016 and 2017.
...
```{r}
chisq.test(crime_weather_3$not_rainy_or_snowy,crime_weather_3$rainy, crime_weather_3$rainy_and_snowy, crime_weather_3$snowy)
```
